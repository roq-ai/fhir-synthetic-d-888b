...
import { getAdminPortals } from 'apiSdk/admin-portals';
import { AdminPortalInterface } from 'interfaces/admin-portal';
import { getHealthData } from 'apiSdk/health-data';
import { HealthDataInterface } from 'interfaces/health-data';

import { HealthDataListPage as ShowCaseList } from 'pages/health-data';
...
function DashboardPage() {
  ...
  const adminPortalStat = useSWR<PaginatedInterface<AdminPortalInterface>>(
    () => `/admin-portals`,
    () => getAdminPortals({ limit: 1, offset: 0 }),
  );

  const healthDataStat = useSWR<PaginatedInterface<HealthDataInterface>>(
    () => `/health-data`,
    () => getHealthData({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Admin Portals',
      result: adminPortalStat?.data?.totalCount,
      isLoading: adminPortalStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Health Data',
      result: healthDataStat?.data?.totalCount,
      isLoading: healthDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`health-data`} passHref legacyBehavior>
                  <Button

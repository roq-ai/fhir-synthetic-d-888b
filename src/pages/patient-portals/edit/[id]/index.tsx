...
import { getPatientPortalById, updatePatientPortalById } from 'apiSdk/patient-portals';
import { patientPortalValidationSchema } from 'validationSchema/patient-portals';
import { PatientPortalInterface } from 'interfaces/patient-portal';
import { UserInterface } from 'interfaces/user';
import { getUsers } from 'apiSdk/users';

function PatientPortalEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<PatientPortalInterface>(
    () => (id ? `/patient-portals/${id}` : null),
    () => getPatientPortalById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PatientPortalInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePatientPortalById(id, values);
      mutate(updated);
      resetForm();
      router.push('/patient-portals');
    } catch (error: any) {
  ...
  const formik = useFormik<PatientPortalInterface>({
    initialValues: data,
    validationSchema: patientPortalValidationSchema,
          ...
          items={[
            {
              ...
              link: '/patient-portals',
            },
            {
              label: 'Update Patient Portal',
              isCurrent: true,
            },
          ]}
        />
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <Flex justifyContent={'flex-start'}>
            ...
            <Button
              ...
              gap="0.5rem"
              mr="4"
              onClick={() => router.push('/patient-portals')}
              _hover={{
                bg: 'neutral.transparent',
                color: 'neutral.main',
              }}

...
import { getAdminPortalById, updateAdminPortalById } from 'apiSdk/admin-portals';
...
function AdminPortalEditPage() {
  ...
  const handleSubmit = async (values: AdminPortalInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAdminPortalById(id, values);
      mutate(updated);
      resetForm();
